...
function rowToProof(row: {
  ...
  return {
    ...
    collections: Array.isArray(row.collections) ? (row.collections as string[]) : undefined,
  };
}
...
export async function listShipsForFeed(
  limit = 100,
  cursor?: string,
  ...
  // Cursor pagination (timestamp-based)
  if (cursor) {
  ...
  if (opts?.since) {
    query = query.gte("timestamp", opts.since);
  }
  if (opts?.until) {
    query = query.lte("timestamp", opts.until);
  }
...
export async function insertShip(proof: Proof): Promise<Proof> {
  ...
  const row = {
    ...
    collections: proof.collections ?? null,
  };
