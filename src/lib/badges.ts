...
function uniqueArtifactTypes(proofs: Proof[]): Set<string> {
  return new Set(proofs.map((r) => r.artifact_type));
}
...
export const BADGE_CATALOG: BadgeDefinition[] = [
  // ——— Tier 1: Bronze ———
  { id: "first-ship", label: "First Launch", description: "Land your first proof", tier: 1, condition: (a) => a.total_proofs >= 1 },
  { id: "getting-started", label: "Getting Started", description: "3 launches landed", tier: 1, condition: (a) => a.total_proofs >= 3 },
  { id: "rookie", label: "Rookie Launcher", description: "5 launches landed", tier: 1, condition: (a) => a.total_proofs >= 5 },
  { id: "ten-ships", label: "Ten Launches", description: "10 launches landed", tier: 1, condition: (a) => a.total_proofs >= 10 },
  ...
  { id: "link-sharer", label: "Link Sharer", description: "Shared a link artifact", tier: 1, condition: (_, p) => uniqueArtifactTypes(p).has("link") },
  { id: "single-type", label: "Focused", description: "Launched one artifact type so far", tier: 1, condition: (_, p) => p.length >= 1 && uniqueArtifactTypes(p).size === 1 },
  // ——— Tier 2: Silver ———
  { id: "twenty", label: "Twenty", description: "20 launches landed", tier: 2, condition: (a) => a.total_proofs >= 20 },
  { id: "top-shipper", label: "Top Launcher", description: "15+ launches landed", tier: 2, condition: (a) => a.total_proofs >= 15 },
  { id: "pro-shipper", label: "Pro Launcher", description: "25 launches landed", tier: 2, condition: (a) => a.total_proofs >= 25 },
  { id: "multi-type", label: "Multi-Type", description: "Launched 2 different artifact types", tier: 2, condition: (_, p) => uniqueArtifactTypes(p).size >= 2 },
  { id: "repo-ranger", label: "Repo Ranger", description: "Landed a GitHub repo", tier: 2, condition: (_, p) => uniqueArtifactTypes(p).has("github") },
  { id: "contract-crusher", label: "Contract Crusher", description: "Landed a smart contract", tier: 2, condition: (_, p) => uniqueArtifactTypes(p).has("contract") },
  ...
  { id: "thirty", label: "Thirty", description: "30 launches landed", tier: 2, condition: (a) => a.total_proofs >= 30 },
  // ——— Tier 3: Gold ———
  { id: "fifty", label: "Fifty", description: "50 launches landed", tier: 3, condition: (a) => a.total_proofs >= 50 },
  { id: "full-stack", label: "Full Stack", description: "Launched 3+ artifact types", tier: 3, condition: (_, p) => uniqueArtifactTypes(p).size >= 3 },
  { id: "dapp-builder", label: "dApp Builder", description: "Landed a dApp", tier: 3, condition: (_, p) => uniqueArtifactTypes(p).has("dapp") },
  { id: "ipfs-pioneer", label: "IPFS Pioneer", description: "Landed on IPFS", tier: 3, condition: (_, p) => uniqueArtifactTypes(p).has("ipfs") },
  { id: "arweave-archivist", label: "Arweave Archivist", description: "Landed on Arweave", tier: 3, condition: (_, p) => uniqueArtifactTypes(p).has("arweave") },
  { id: "century", label: "Century", description: "100 launches landed", tier: 3, condition: (a) => a.total_proofs >= 100 },
  ...
  { id: "veteran", label: "Veteran", description: "60+ days active with 20+ launches", tier: 3, condition: (a) => daysSince(a.first_seen) >= 60 && a.total_proofs >= 20 },
  ...
  { id: "jack-of-all", label: "Jack of All", description: "Launched 4+ artifact types", tier: 3, condition: (_, p) => uniqueArtifactTypes(p).size >= 4 },
  { id: "everest", label: "Everest", description: "50 launches — halfway to century", tier: 3, condition: (a) => a.total_proofs >= 50 },
  // ——— Tier 4: Platinum ———
  { id: "fleet-captain", label: "Fleet Captain", description: "250 launches landed", tier: 4, condition: (a) => a.total_proofs >= 250 },
  { id: "docking-master", label: "Landing Master", description: "500 launches landed", tier: 4, condition: (a) => a.total_proofs >= 500 },
  { id: "legend", label: "Legend", description: "1000 launches landed", tier: 4, condition: (a) => a.total_proofs >= 1000 },
  ...
  { id: "littleships-sage", label: "LittleShips Sage", description: "100+ launches and 4+ types", tier: 4, condition: (a, p) => a.total_proofs >= 100 && uniqueArtifactTypes(p).size >= 4 },
  { id: "completionist", label: "Completionist", description: "Launched all artifact types", tier: 4, howToEarn: "Land at least one proof of each type: github, contract, dapp, ipfs, arweave, and link.", condition: (_, p) => uniqueArtifactTypes(p).size >= 6 },
  { id: "titan", label: "Titan", description: "200 launches landed", tier: 4, condition: (a) => a.total_proofs >= 200 },
  { id: "immortal", label: "Immortal", description: "500 launches — landing master", tier: 4, condition: (a) => a.total_proofs >= 500 },
